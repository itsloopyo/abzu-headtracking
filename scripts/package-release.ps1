...
# named the log file AbzuHeadTracking.log while the mod writes HeadTracking.log,
# so a launcher install and a manual install logged to different names.
# Regenerate it from the file the ZIP ships. Write the committed manifest, not
# just the staged copy - that is what conformance.ps1's manifest-seed check
# reads, and release.ps1 commits it with the version bump. Raw-text replacement
# rather than a JSON round-trip so the file's formatting survives.
